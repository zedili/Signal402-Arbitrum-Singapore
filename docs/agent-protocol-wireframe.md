...
sequenceDiagram
    ...
    S-->>H: Structured report and receipt
```
...
sign the user's registry transaction.
...
2. Tests for the unpaid 402 response, paid retry, stable hashes, changed-content
   hashes, idempotency, and every pre-settlement failure path.
